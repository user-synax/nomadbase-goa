...
import { ArrowUp, Trash2, MessageCircle, Eye } from "lucide-react";
...
export async function generateMetadata({ params }) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/community/${params.threadId}`, {
      cache: 'no-store'
    });
...
export default function ThreadDetailPage({ params }) {
  ...
  const fetchThread = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/community/${params.threadId}`);
      const data = await response.json();
  ...
  const handleUpvote = async () => {
    try {
      const response = await fetch(`/api/community/${params.threadId}/upvote`, {
        method: "POST",
      });
      const data = await response.json();
      ...
      toast.error(error.message || "Failed to toggle upvote");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#171717] pt-16">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[#898989]">•</span>
              <span className="text-sm text-[#898989]" style={{ fontFamily: "Circular, custom-font, Helvetica Neue, Helvetica, Arial, sans-serif" }}>
                Posted {getTimeAgo(thread.createdAt)}
              </span>
            </div>
          </div>
        ...
        {/* Reply Box */}
        <ReplyBox threadId={params.threadId} onReplyAdded={handleReplyAdded} />
      </div>
    </div>
  );
}
