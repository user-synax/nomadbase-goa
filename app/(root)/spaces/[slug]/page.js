...
export default function SpaceDetailPage({ params }) {
  ...
  useEffect(() => {
    ...
    async function fetchSpace() {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/spaces/${params.slug}`);
        if (!res.ok) {
          notFound();
        }
    ...
    async function fetchReviews() {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/spaces/${params.slug}/reviews`);
        const data = await res.json();
        setReviews(data);
      } catch (error) {
    ...
    // Check auth status (simplified - in real app, use session)
  ...
  if (!space) {
    return <div className="bg-[#171717] min-h-screen">Loading...</div>;
  }
...
export async function generateMetadata({ params }) {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/spaces/${params.slug}`, { cache: 'no-store' });
    if (!res.ok) return {};
